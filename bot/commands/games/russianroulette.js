...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (await getSet(interaction, lang)) return;
        await addSet(interaction.user.id);

        betAmount = betAmount.toLowerCase() === "a" ? playerData.balance : Number(betAmount);
        if (betAmount <= 0 || await maxBet(playerData, betAmount, lang, interaction, client)) {
