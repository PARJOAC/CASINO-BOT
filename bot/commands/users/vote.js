...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const cooldownTime = 43200000; // 12 horas en milisegundos
...
/*
    case 1:
      const items = [
        ...
        { emoji: "💤", name: "siesta" },
        { emoji: "🍾", name: "cava" },
        { emoji: "🎶", name: "castanuelas" },
        { emoji: "👒", name: "sombrero" },
        { emoji: "🏰", name: "sagradaFamilia" },
        { emoji: "⚽", name: "soccerBall" },
        { emoji: "🍷", name: "wine" },
        { emoji: "☀️", name: "sol" },
        { emoji: "🇪🇸", name: "spanishFlag" },
        { emoji: "🧉", name: "mate" },
      ];
      const randomItem = items[Math.floor(Math.random() * items.length)];
      const itemKey = randomItem.name;
      playerData.swag[itemKey] = (playerData.swag[itemKey] || 0) + 1;
      await playerData.save();
      message = lang.receivedItem.replace(
        "{item}",
        `\`x1 ${randomItem.emoji} ${itemKey}\``
      );
      break;
*/
    case 1:
